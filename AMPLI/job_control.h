...
enum job_state { FOREGROUND, BACKGROUND, STOPPED, RESPAWNED, STOP_RESPAWN };
static char* status_strings[] = { "Suspended", "Signaled", "Exited", "Continued"};
static char* state_strings[] = { "Foreground", "Background", "Stopped", "Respawnable", "Stop Respawnable" };
...
job * new_job(pid_t pid, const char * command, int nargs, char ** args, enum job_state state);
...
#define new_list(name) \
    ({ \
        char *args[] = {name, NULL}; /* name debe ser const char* */ \
        new_job(0, name, 0, args, FOREGROUND); /* Llamada a new_job */ \
    })
